...
enum TPWechatAliPayInfoPageType { weChat, aliPay }

class TPWechatAliPayInfoPage extends StatefulWidget {
  TPWechatAliPayInfoPage({Key key, @required this.type,this.walletAddress,this.paymentModel}) : super(key: key);

  final String walletAddress;

  final TPWechatAliPayInfoPageType type;

  final TPPaymentModel paymentModel;

  @override
  _TPWechatAliPayInfoPageState createState() =>
      _TPWechatAliPayInfoPageState();
}

class _TPWechatAliPayInfoPageState extends State<TPWechatAliPayInfoPage> {
  List titles;
  ...
  void initState() {
    ...
    if (widget.type == TPWechatAliPayInfoPageType.weChat) {
   ...
   void createPayment(){
    ...
    if(_pramaterModel.account.length == 0){
      String msg = widget.type == TPWechatAliPayInfoPageType.weChat ? '请填写微信账号' : '请填写支付宝账号';
      Fluttertoast.showToast(
                      msg: msg,
                      toastLength: Toast.LENGTH_SHORT,
                      timeInSecForIosWeb: 1);
    ...
    setState(() {
      ...
      _manager.createPayment(_pramaterModel, (){
        String msg = widget.type == TPWechatAliPayInfoPageType.weChat ? '添加微信号成功' : '添加支付宝成功';
        Fluttertoast.showToast(
                      msg: msg,
                      toastLength: Toast.LENGTH_SHORT,
                      timeInSecForIosWeb: 1);
  ...
  void updatePayment(){
    if(_pramaterModel.realName.length == 0){
      ...
      _manager.updatePayment(_pramaterModel, (){
        String msg = widget.type == TPWechatAliPayInfoPageType.weChat ? '修改微信号成功' : '修改支付宝成功';
        Fluttertoast.showToast(
                      msg: msg,
                      toastLength: Toast.LENGTH_SHORT,
                      timeInSecForIosWeb: 1);
