...
class _TPPursePageState extends State<TPPursePage>
  ...
  void _registerRefreshEvent() {
    _refreshSubscription =
        eventBus.on<TPRefreshFirstPageEvent>().listen((event) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        body: LoadingOverlay(
          ...
          child: SmartRefresher(
            ...
            header: WaterDropHeader(
              complete:
                  Text(I18n.of(navigatorKey.currentContext).refreshComplete),
            ),
        ...
        appBar: CupertinoNavigationBar(
            ...
            middle: Text(
              I18n.of(context).commonPageTitle,
            ...
            trailing: Container(
                ...
                child: Row(
                    ...
                    children: <Widget>[
                      CupertinoButton(
                        child: Icon(
                          IconData(0xe6fe, fontFamily: 'appIconFonts'),
                          color: Colors.white,
                        ),
                        ...
                        onPressed: () {
                          Navigator.push(
                              context,
                              MaterialPageRoute(
                                  builder: (contenxt) => TPScanQrCodePage(
                                        scanCallBack: (String qrCode) {
                                          _qrCodeModelManager.scanQRCodeResult(
                                              qrCode, (TPQRcodeCallBackModel
                                                  callBackModel) async {
                                            if (callBackModel.type ==
                                                QRCodeType.redEnvelope) {
                                              _getRedEnvelopeInfo(
                                                  callBackModel.data);
                                            } else if (callBackModel.type ==
                                                QRCodeType.transfer) {
                                              String toWalletAddres =
                                                  callBackModel.data;
                                              navigatorKey.currentState
                                                ...
                                                QRCodeType.inviteCode) {
                                              String inviteCode =
                                                  callBackModel.data;
                                              String token = await TPDataManager
                                                  .instance
                                                  .getAcceptanceToken();
                                              if (token == null) {
                                                Navigator.push(
  ...
  Widget _getListViewItem(BuildContext context, int index) {
    if (index == 0) {
      return TPPurseHeaderCell(
        totalAmount: _totalAmount,
      );
    } else if (index == 1) {
      return TPPurseInfoListView(
        walletList: _dataSource,
        didClickItemCallBack: (int dataIndex) {
          TPWalletInfoModel model = _dataSource[dataIndex];
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) {
                return TPMyPursePage(
                  wallet: model.wallet,
                  changeNameSuccessCallBack: (String name) {
                    setState(() {
  ...
  void _createPurse(BuildContext context) async {
    ...
    if (status == PermissionStatus.denied) {
      ...
      Fluttertoast.showToast(
          msg: I18n.of(navigatorKey.currentContext)
              .PleaseTurnOnTheStoragePermissions);
      return;
    }
  ...
  void _getRedEnvelopeInfo(String redEnvelopeId) {
    ...
    _manager.getRedEnvelopeInfo(redEnvelopeId,
        (TPDetailRedEnvelopeModel redEnvelopeModel) {
      if (mounted) {
        setState(() {
          _isLoading = false;
        });
      ...
      showDialog(
          context: context,
          builder: (context) => TPUnopenRedEnvelopeAlertView(
  ...
  void _recieveRedEnvelope(String walletAddress, String redEnvelopeId) {
    ...
    _manager.recieveRedEnvelope(redEnvelopeId, walletAddress,
        (int recieveLogId) {
      if (mounted) {
        setState(() {
          _isLoading = false;
      ...
      Navigator.push(
          context,
          MaterialPageRoute(
              builder: (context) => TPDetailRecieveRedEnvelopePage(
                    receiveLogId: recieveLogId,
                  )));
    }, (TPError error) {
      if (mounted) {
        setState(() {
  ...
  Widget _getActionWidget() {
    ...
    if (isHaveImport) {
      return Padding(
          ...
          child: getButton(
              () => () {},
              I18n.of(context).createWalletBtnTitle,
          ...
          child: Row(
              ...
              children: <Widget>[
                ...
                getButton(
                    ...
                    I18n.of(context).importWalletBtnTitle,
                    MediaQuery.of(context).size.width,
  ...
  Widget getButton(Function didClickCallBack, String title, double scrrenWidth,
    ...
    return Container(
      ...
      child: CupertinoButton(
        ...
        padding: EdgeInsets.zero,
        borderRadius:
            BorderRadius.all(Radius.circular(ScreenUtil().setHeight(40))),
        child: Text(title,
